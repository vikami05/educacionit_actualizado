...
@app.route('/campus')
def inicio_campus():
    # Podés pasar datos de sesión si querés
...
def obtener_progreso(id_curso):
    ...
    try:
        ...
        if progreso:
            ...
            return jsonify({
                'existe': False,
                'mensaje': 'No hay progreso guardado'
            }), 200

    except Exception as e:
        print("Error al obtener progreso:", str(e))
        return jsonify({'error': str(e)}), 500



# 2. Guardar progreso (VERSIÓN CORREGIDA)
...
def guardar_progreso(id_curso):
    ...
    try:
        ...
        # ✅ NO permitir que se cambie curso_completado desde esta ruta
        # Solo la ruta /completar puede marcar el curso como completado
        
        conn = get_db_connection()
        cursor = conn.cursor(dictionary=True)

        # ✅ PASO 1: Verificar si ya existe progreso
        cursor.execute("""
        ...
        # ✅ PASO 2: Si el curso ya está completado, NO sobrescribir
        if registro_existente and registro_existente['curso_completado'] == 1:
            print(f"⚠️ Curso {id_curso} ya completado - No se actualizará curso_completado")
            curso_completado_final = 1  # Mantener completado
        else:
            curso_completado_final = 0  # Mantener en progreso

        datos_json = json.dumps(datos_progreso) if datos_progreso else None

        # ✅ PASO 3: Guardar progreso SIN afectar curso_completado si ya está en 1
        cursor.execute("""
        ...
        print("Error al guardar progreso:", str(e))
        return jsonify({'error': str(e)}), 500



# 3. Completar curso (VERSIÓN MEJORADA)
...
def completar_curso(id_curso):
    ...
    try:
        ...
        print(f"🎓 Intentando completar curso {id_curso} para usuario {usuario_id}")

        cursor.execute("""
            UPDATE progreso 
            SET curso_completado = 1,
                fecha_ultima_actualizacion = CURRENT_TIMESTAMP
            WHERE id_usuario = %s AND id_curso = %s
        """, (usuario_id, id_curso))
        ...
        if cursor.rowcount > 0:
            print(f"✅ Curso {id_curso} completado exitosamente para usuario {usuario_id}")
            cursor.close()
            conn.close()
            return jsonify({
            ...
            print(f"⚠️ No se encontró progreso previo para curso {id_curso} y usuario {usuario_id}")
            
            # ✅ Si no existe progreso, crearlo como completado
            cursor.execute("""
            ...
            print(f"✅ Progreso creado y curso {id_curso} marcado como completado")
            return jsonify({
                'mensaje': 'Curso completado exitosamente',
                'completado': True
            }), 200
        ...
        print(f"❌ Error al completar curso {id_curso}:", str(e))
        return jsonify({'error': str(e)}), 500
...
def reiniciar_progreso(id_curso):
    ...
    try:
        ...
        print("Error al reiniciar progreso:", str(e))
        return jsonify({'error': str(e)}), 500
...
def registrar_nota_final(id_curso):
    print(f"🎯 Entró al endpoint /api/curso/{id_curso}/nota")  # 👈 Agregá esto
    usuario_id = session.get('usuario_id')
    if not usuario_id:
    ...
    # Guardar nota final en progreso
    cursor.execute("""
        UPDATE progreso
    ...
    if nota >= 7:
        ...
        # Buscar datos del usuario y curso
        cursor.execute("""
        ...
        if not cert:
            ...
            cursor.execute("""
                INSERT INTO certificados (id_inscripcion, codigo_unico)
                VALUES (%s, %s)
            """, (info['id_inscripcion'], codigo_unico))
            conn.commit()
...
def crear_reembolso():
    try:
        data = request.get_json()
        id_curso = data.get('id_pago')  # sigue enviando el front
        ...
        # 🔹 Buscar id_usuario según el email
        ...
        # 🔹 Buscar pago aprobado de ese usuario para ese curso
        cursor.execute("""
            SELECT id_pago 
            FROM pagos 
        ...
        if not pago:
            cursor.close()
            conn.close()
            return jsonify({'error': 'No se encontró un pago aprobado para este curso por el usuario'}), 404

        id_pago = pago['id_pago']

        # 🔹 Insertar reembolso
        cursor.execute("""
            INSERT INTO reembolsos (id_pago, motivo)
            VALUES (%s, %s)
        """, (id_pago, motivo))
        conn.commit()
        cursor.close()
        ...
        print(f"💬 Reembolso solicitado por {email_usuario} - Curso #{id_curso} (Pago #{id_pago})")
...
def admin_logout():
    ...
    return redirect(url_for('login'))  # ahora apunta al login general
...
def actualizar_reembolso(id_reembolso):
    if not session.get('es_admin'):
        return jsonify({'error': 'No autorizado'}), 403
    try:
        data = request.get_json()
        ...
        cursor.close()
        conn.close()
        return jsonify({'mensaje': f'Reembolso actualizado a {nuevo_estado}'}), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 500
